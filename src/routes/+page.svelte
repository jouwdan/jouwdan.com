<div
	class="container px-12 rounded-xl py-6 lg:py-12 mx-auto justify-center items-center bg-gradient-to-br from-primary-500 to-secondary-500 text-surface-900"
>
		<div class="flex flex-col-reverse lg:flex-row">
			<div class="my-auto lg:w-1/2 lg:pl-24">
				<h1 class="font-bold mb-4">Hey, I'm Jordan!</h1>
				<h3>
					I'm a creative full stack developer based in Ireland. I love tinkering with web tech and am a big fan of open source. Also, I'm a bit of a tech nerd (but hey, who isn't these days?).
				</h3>
				<a class="btn variant-ghost-primary mt-4" href="/about">About Me</a>
				<a class="btn variant-ghost-secondary mt-4" href="/contact">Get in Touch</a>
			</div>
			<img src="images/waving-avatar.png" alt="avatar of Jordan waving" class="mx-auto pb-12" />
		</div>
</div>
